#!/usr/bin/env bash
# Common test setup for all Anvil bats tests
...
# Project root (the anvil plugin repo)
PLUGIN_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"

# Scripts under test
SETUP_SCRIPT="${PLUGIN_ROOT}/scripts/setup-anvil.sh"
STOP_HOOK="${PLUGIN_ROOT}/hooks/stop-hook.sh"

# Create isolated test directory (each test gets its own)
setup_test_dir() {
  TEST_DIR="$(mktemp -d "${BATS_TEST_TMPDIR}/anvil-test.XXXXXX")"
  mkdir -p "${TEST_DIR}/.claude"

  # Initialize a minimal git repo so --diff tests work
  git init --quiet "${TEST_DIR}" 2>/dev/null || true
}
...
state_file() {
  echo "${TEST_DIR}/.claude/anvil-state.local.md"
}

# Helper: get the result file path for a test dir
result_file() {
  echo "${TEST_DIR}/.claude/anvil-result.local.md"
}
