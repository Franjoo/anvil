...
allowed-tools: ["Bash(test -f .claude/anvil-state.local.md:*)", "Bash(rm .claude/anvil-state.local.md)", "Read(.claude/anvil-state.local.md)"]
...
1. Check if `.claude/anvil-state.local.md` exists using Bash: `test -f .claude/anvil-state.local.md && echo "EXISTS" || echo "NOT_FOUND"`
